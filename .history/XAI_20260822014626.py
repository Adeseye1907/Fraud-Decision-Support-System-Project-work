...
def consume_transactions():
    ...
    for message in consumer:
        ...
        print("TRANSACTION RECEIVED")
        print("=" * 60)
        print("Transaction ID:", transaction_data["transaction_id"])

        model_input = pd.DataFrame(
            [{column: transaction_data[column] for column in feature_columns}]
        )

        prediction = rf_model.predict(model_input)[0]
        probability = rf_model.predict_proba(model_input)[0][1]
        ...
        try:
            ...
            print("\n--- TOP 5 SHAP FEATURES ---")
            print(shap_df[["Feature", "Feature_Value", "SHAP_Value"]].head(5).to_string(index=False))
