...
# ============================================================
    ...
    st.markdown(
        ...
        [data-testid="stMetricLabel"] * {
            color: #9ca3af !important; /* Visible light grey label */
        ...
        [data-testid="stMetricValue"] * {
            color: #ffffff !important; /* Bold high-contrast white value */
...
# Custom styling to ensure subheaders have high contrast, correct z-index, and clear visibility
st.markdown(
    """
    <style>
    /* Force high-visibility and proper spacing on all subheaders */
...
# ---------------------------------------------------------
# Prediction Outcome Summary Section
# ---------------------------------------------------------
st.divider()
...
with p1:
    st.metric(
        ...
        value=f"{int(counts.get('FRAUD', 0)):,}",
    )

with p2:
    st.metric(
        label="Predicted Legitimate",
        value=f"{int(counts.get('LEGITIMATE', 0)):,}",
    )

# ---------------------------------------------------------
# Historical Prediction Results Section
# ---------------------------------------------------------
st.divider()
...
available_columns = [
    col for col in display_columns if col in historical_predictions_page.columns
]
