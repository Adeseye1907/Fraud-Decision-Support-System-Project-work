...
def create_kafka_consumer():
    ...
    try:
        ...
        consumer = KafkaConsumer(
            ...
            # Start from existing messages when testing
            ...
            # Do not block Streamlit indefinitely
        ...
        for topic_partition, records in messages.items():
            ...
            for message in records:
                ...
                if isinstance(transaction, dict):
                    ...
                    # Avoid storing the exact same Kafka
...
# ============================================================
# LOAD MODEL + SHAP EXPLAINER FOR MANUAL/BATCH PREDICTION
# ============================================================
...
with interpretation1:
    ...
    st.write(
        ...
        in prioritizing suspicious transactions,
...
# ============================================================
# PREDICT FRAUD (Manual Entry + Batch Upload)
# ============================================================
    ...
    st.write(
        """
        Submit a transaction manually, or upload a CSV of
        transactions, to check the models fraud prediction
        and see the top contributing factors behind each result.
        """
    )
    ...
    tab_manual, tab_batch = st.tabs(
        ["✍️ Manual Entry", "📁 Batch CSV Upload"]
    )

    # --------------------------------------------------------
    # HELPER: run prediction + SHAP on a single-row DataFrame
    # --------------------------------------------------------
    ...
    with tab_manual:
        ...
        st.caption(
            "V1–V28 are anonymised (PCA-transformed) transaction "
            "features from the original dataset. If unsure, leave "
            "at default (0.0) to test a typical/neutral transaction."
        )
        ...
        with st.form("manual_prediction_form"):
            ...
            with col_a:
                input_time = st.number_input("Time (scaled)", value=0.0, format="%.6f")
                input_amount = st.number_input("Amount (scaled)", value=0.0, format="%.6f")
        ...
        if submitted:
            ...
            row = {"Time": input_time, "Amount": input_amount}
            row.update(v_inputs)
            ...
            top5["Direction"] = np.where(
                top5["SHAP_Value"] >= 0, "↑ Pushes toward Fraud", "↓ Pushes toward Legitimate"
            )
    ...
    with tab_batch:
        ...
        st.caption(
            ...
            "Time, V1–V28, Amount (30 columns total, same order "
            "the model was trained on)."
        )

        uploaded_file = st.file_uploader(
            "Choose a CSV file", type="csv", key="batch_predict_upload"
        )
        ...
        if uploaded_file is not None:
            ...
            if missing_cols:
                ...
                with bc3:
                    st.metric("Fraud Rate", f"{(fraud_batch/total_batch*100):.2f}%")
...
# ============================================================
    ...
    st.write(
        ...
        support transaction investigation, prioritization
