...
# ============================================================
    ...
    # Scoped styling for clean emerald tabs, crisp typography, and high-visibility tables
    st.markdown(
        """
        <style>
    ...
    # Ingest incoming Kafka stream records before rendering
    read_kafka_transactions()

    st.markdown(
        '<div class="main-title">Transaction Explorer</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="subtitle">Investigate live stream transactions, Kafka events, and historical dataset classifications.</div>',
        unsafe_allow_html=True,
    )

    # Resolve available dataset for audit
    explorer_dataset = None
    if "historical_predictions" in globals() and historical_predictions is not None:
        explorer_dataset = historical_predictions
    elif os.path.exists("historical_predictions.csv"):
        try:
            explorer_dataset = pd.read_csv("historical_predictions.csv")
        except Exception:
            explorer_dataset = None
    elif os.path.exists("creditcard.csv"):
        try:
            explorer_dataset = pd.read_csv("creditcard.csv")
        except Exception:
            explorer_dataset = None

    tab_live, tab_hist = st.tabs(["Live Stream / Session Explorer", "Historical Dataset Filter"])

    # --- TAB 1: LIVE STREAM INVESTIGATION ---
    with tab_live:
        st.subheader("Transaction ID Lookup")
        st.markdown(
        ...
        if query:
            ...
            if matches:
                st.success(f"✓ {len(matches)} transaction(s) found in live stream.")
                st.dataframe(pd.DataFrame(matches), use_container_width=True, hide_index=True)
            else:
                st.warning(f"No active stream transaction matching '{query}' found.")
        elif transactions:
            st.caption(f"Showing all {len(transactions)} active stream transactions")
            st.dataframe(pd.DataFrame(transactions), use_container_width=True, hide_index=True)
        else:
            st.info("No live stream transactions recorded yet. Kafka consumer is actively polling on 'fraud-predictions'.")

    # --- TAB 2: HISTORICAL DATASET AUDIT ---
    with tab_hist:
        st.subheader("Historical Records Audit")
        st.markdown(
            '<div class="explorer-caption">Filter and inspect recorded transactions by minimum amount or classification label.</div>',
            unsafe_allow_html=True,
        )

        if explorer_dataset is not None and not explorer_dataset.empty:
            ...
            with col_f1:
                search_amount = st.number_input(
                    ...
                    value=0.0,
                    step=10.0,
                    key="hist_search_amount",
                )
            with col_f2:
                filter_choice = st.selectbox(
                    "Classification Filter",
                    options=["All", "Fraud Only", "Legitimate Only"],
                    key="hist_class_filter",
                )

            filtered_df = (
                explorer_dataset[explorer_dataset["Amount"] >= search_amount]
                if "Amount" in explorer_dataset.columns
                else explorer_dataset
            )

            # Support both raw "Class" column and model output "prediction" column
            if "Class" in filtered_df.columns:
                if filter_choice == "Fraud Only":
                    filtered_df = filtered_df[filtered_df["Class"] == 1]
                elif filter_choice == "Legitimate Only":
                    filtered_df = filtered_df[filtered_df["Class"] == 0]
            elif "prediction" in filtered_df.columns:
                if filter_choice == "Fraud Only":
                    filtered_df = filtered_df[filtered_df["prediction"] == "FRAUD"]
                elif filter_choice == "Legitimate Only":
                    filtered_df = filtered_df[filtered_df["prediction"] == "LEGITIMATE"]

            st.caption(f"Displaying {min(len(filtered_df), 500):,} of {len(filtered_df):,} matching records")
            st.dataframe(filtered_df.head(500), use_container_width=True, height=450)
        else:
            st.warning("Historical dataset is currently unavailable.")
...
# ============================================================
    ...
    st.markdown(
        '<div class="main-title">Trends & Monitoring</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="subtitle">Monitor transaction activity and fraud patterns across the real-time Kafka transaction stream.</div>',
        unsafe_allow_html=True,
    )
    ...
    if transactions:
        ...
        # ----------------------------------------------------
        # TRANSACTION STREAM KPIS
        # ----------------------------------------------------
        k1, k2, k3 = st.columns(3)
        total_live = len(df)
        fraud_live = int((df["prediction"] == "FRAUD").sum()) if "prediction" in df.columns else 0
        live_rate = (fraud_live / total_live * 100) if total_live > 0 else 0.0

        with k1:
            st.metric("Total Streamed", f"{total_live:,}")
        with k2:
            st.metric("Flagged Fraud", f"{fraud_live:,}")
        with k3:
            st.metric("Stream Fraud Rate", f"{live_rate:.2f}%")
        ...
        if "timestamp" in df.columns:
            ...
            if not valid_time_df.empty:
                chart = valid_time_df.set_index("timestamp").resample("1min").size()

                st.subheader("Transaction Volume (1-Min Window)")
                st.line_chart(chart, use_container_width=True)

                if "prediction" in valid_time_df.columns:
                    st.subheader("Stream Classification Breakdown")
                    st.bar_chart(valid_time_df["prediction"].value_counts(), use_container_width=True)
            else:
                st.info("Timestamp data is not formatted as valid datetimes.")
        else:
            st.info("Timestamp information is not available in the current Kafka stream.")

        st.divider()
        st.subheader("Stream Log Data")
        st.dataframe(df, use_container_width=True, hide_index=True)

    else:
        st.info("Waiting for transaction data from Kafka...")
