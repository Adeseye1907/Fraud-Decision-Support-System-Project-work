...
# ============================================================
...
elif page == "End-to-End DSS Workflow" or (locals().get("current_page") == "End-to-End DSS Workflow"):
    ...
    st.markdown(
        ...
        <div class="dashboard-header" style="display: flex; justify-content: space-between; align-items: center; gap: 20px;">
            <div style="flex: 1; padding-right: 20px;">
                <div class="workflow-title-main">End-to-End DSS Architecture & Workflow</div>
                <div class="workflow-sub-main">
                    Integrated reference topology connecting real-time streaming, explainable AI, adaptive drift detection, and human-in-the-loop decision support.
                </div>
            </div>
    ...
    # --------------------------------------------------------
    # 1. LIVE SYSTEM COMPONENT TELEMETRY
    # --------------------------------------------------------
    ...
    with s1:
        st.markdown(
            ...
            <div class="sys-status-card">
                ...
                <div class="sys-status-val" style="color: {'#15803d' if kafka_online else '#dc2626'} !important;">
    ...
    with s2:
        st.markdown(
            ...
            <div class="sys-status-card">
                ...
                <div class="sys-status-val" style="color: {'#15803d' if model_loaded else '#dc2626'} !important;">
                    {'● LOADED' if model_loaded else '○ OFFLINE'}
                </div>
    ...
    with s3:
        st.markdown(
            ...
            <div class="sys-status-card">
                ...
                <div class="sys-status-val" style="color: {'#15803d' if lime_ready else '#dc2626'} !important;">
                    {'● ACTIVE' if lime_ready else '○ OFFLINE'}
                </div>
    ...
    with s4:
        st.markdown(
            ...
            <div class="sys-status-card">
                ...
                <div class="sys-status-val" style="color: {drift_color} !important; font-size: 20px;">
                    ● {drift_status}
                </div>
    ...
    # --------------------------------------------------------
    # 2. END-TO-END DECISION CHAIN TOPOLOGY
    # --------------------------------------------------------
    st.markdown("""<div class="section-title">Operational Decision Chain</div>""", unsafe_allow_html=True)

    st.markdown(
        """
        <div class="pipeline-chain-box">
            <div style="font-size: 12px; font-weight: 700; color: #94a3b8; text-transform: uppercase; margin-bottom: 12px;">
                Sequential Flow of Operational Intelligence
            </div>
            <div>
                <span class="chain-node">1. Ingestion (Kafka)</span>
                <span class="chain-arrow">➔</span>
                <span class="chain-node">2. Prediction (RF)</span>
                <span class="chain-arrow">➔</span>
                <span class="chain-node">3. Explanation (XAI)</span>
                <span class="chain-arrow">➔</span>
                <span class="chain-node">4. Monitoring (Drift)</span>
                <span class="chain-arrow">➔</span>
                <span class="chain-node">5. Review (Feedback)</span>
                <span class="chain-arrow">➔</span>
                <span class="chain-node">6. Learning Pipeline</span>
                <span class="chain-arrow">➔</span>
                <span class="chain-node">7. Model Adaptation</span>
                <span class="chain-arrow">➔</span>
                <span class="chain-node">8. Value Framing</span>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    # --------------------------------------------------------
    # 3. COMPLETE ARCHITECTURE SPECIFICATION TABLE
    # --------------------------------------------------------
    st.markdown("""<div class="section-title">Framework Component Matrix</div>""", unsafe_allow_html=True)

    workflow_matrix = [
        {"Stage": "01. Historical Ingestion", "Component": "Baseline Dataset", "Subsystem": "creditcard.csv Pipeline", "Operational Function": "Provides historical ground-truth benchmark and baseline feature distributions.", "Status": "Operational"},
        {"Stage": "02. Fraud Scoring", "Component": "Random Forest", "Subsystem": "Ensemble Classifier", "Operational Function": "Generates continuous fraud risk probabilities and binary threat flags.", "Status": "Operational"},
        {"Stage": "03. Stream Transport", "Component": "Apache Kafka", "Subsystem": "Message Broker", "Operational Function": "Streams live transaction payloads and real-time prediction messages asynchronously.", "Status": "Operational"},
        {"Stage": "04. Explainability", "Component": "SHAP & LIME", "Subsystem": "Interpretable AI", "Operational Function": "Extracts local feature attributions and global surrogate rules for investigators.", "Status": "Operational"},
        {"Stage": "05. Adaptive Monitoring", "Component": "Drift Sensors", "Subsystem": "KS-Test & Rate Divergence", "Operational Function": "Monitors stream shifts against historical training distribution in real time.", "Status": "Operational"},
        {"Stage": "06. Model Adaptation", "Component": "Governance Engine", "Subsystem": "Champion-Challenger", "Operational Function": "Evaluates drift triggers and governs retraining iterations without service disruption.", "Status": "Operational"},
        {"Stage": "07. Operational Review", "Component": "Human Feedback", "Subsystem": "Investigator Triage UI", "Operational Function": "Captures investigator decisions (Confirmed Fraud, False Positive, Legitimate).", "Status": "Operational"},
        {"Stage": "08. Feedback Learning", "Component": "Curation Pipeline", "Subsystem": "Data Quality Gate", "Operational Function": "Reconciles reviewer decisions and curates candidate training adaptation datasets.", "Status": "Operational"},
        {"Stage": "09. Financial Framing", "Component": "Cost / Value Engine", "Subsystem": "Utility Modeling", "Operational Function": "Translates confusion metrics into monetary impact and net value protected.", "Status": "Operational"},
        {"Stage": "10. Decision Support", "Component": "Streamlit Framework", "Subsystem": "Multi-Page Portal", "Operational Function": "Unifies real-time monitoring, XAI diagnostics, and governance oversight.", "Status": "Operational"},
    ]

    workflow_df = pd.DataFrame(workflow_matrix)
    st.dataframe(workflow_df, use_container_width=True, hide_index=True)

    # --------------------------------------------------------
    # 4. SYSTEM REFRESH TRIGGER
    # --------------------------------------------------------
    if st.button("🔄 Refresh System Telemetry", use_container_width=True):
        st.rerun()
