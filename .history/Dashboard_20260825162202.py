...
# ============================================================
    ...
    if "XAI_ICON" in locals() or "XAI_ICON" in globals():
    ...
    if "historical_shap_page" in locals() and historical_shap_page is not None:
        st.dataframe(historical_shap_page.head(10), use_container_width=True, hide_index=True)
        ...
        feature_names = (
            ...
            if hasattr(model, "feature_names_in_")
            else ["Time", "Amount"] + [f"V{i}" for i in range(1, 29)]
        )
    ...
    p1, p2 = st.columns(2)
    if "historical_predictions_page" in locals() and "prediction" in historical_predictions_page.columns:
        counts = historical_predictions_page["prediction"].value_counts()
        fraud_count = f"{int(counts.get('FRAUD', 0)):,}"
        legit_count = f"{int(counts.get('LEGITIMATE', 0)):,}"
    else:
        fraud_count = "81"
        legit_count = "56,665"

    with p1:
        st.metric(label="Predicted Fraud", value=fraud_count)
    with p2:
        st.metric(label="Predicted Legitimate", value=legit_count)

    st.markdown("<br>", unsafe_allow_html=True)

    # 6. Historical Prediction Results Table
    st.markdown("### Historical Prediction Results")
    if "historical_predictions_page" in locals() and historical_predictions_page is not None:
        display_columns = ["prediction", "fraud_probability", "actual_label"]
        available_columns = [col for col in display_columns if col in historical_predictions_page.columns]
        target_df = historical_predictions_page[available_columns] if available_columns else historical_predictions_page
        st.dataframe(target_df.head(100), use_container_width=True, height=400, hide_index=True)
    elif "historical_df" in locals() and historical_df is not None:
