...
# ============================================================
    ...
    # --------------------------------------------------------
    # FETCH LIVE TRANSACTIONS SAFELY
    # --------------------------------------------------------
    if "realtime_transactions" not in st.session_state:
    ...
    if view_mode == "Live Real-Time Stream":
        ...
        fraud_count = sum(
            1
            for x in live_txs
            if str(x.get("prediction", "")).upper() == "FRAUD"
        )

        legitimate_count = sum(
            1
            for x in live_txs
            if str(x.get("prediction", "")).upper() == "LEGITIMATE"
        )

        # If transactions exist but prediction labels are missing,
        # keep the dashboard from displaying inconsistent totals.
        if fraud_count + legitimate_count < total_count:
        ...
        for x in live_txs:
            ...
            # Handle probability supplied as decimal
            # e.g. 0.82 -> 82%
            if 0 <= probability <= 1:
                probability *= 100
        ...
        # HISTORICAL BASELINE
        # ----------------------------------------------------
        total_count = int(HISTORICAL_TRANSACTIONS)
        fraud_count = int(HISTORICAL_FRAUD)
        legitimate_count = int(HISTORICAL_LEGITIMATE)

        fraud_rate = float(HISTORICAL_FRAUD_RATE)

        # Historical average risk baseline
        avg_risk = 0.17
    ...
    with k4:
        ...
        # Convert decimal AUC to percentage
        roc_auc_display = (
            MODEL_ROC_AUC * 100
            if MODEL_ROC_AUC <= 1
            else MODEL_ROC_AUC
    ...
    with c1:
        ...
        st.markdown(
            ...
            <div class="dashboard-panel">
                ...
                <div class="panel-subtitle">
                    Current proportion of flagged vs. legitimate events
                </div>
        ...
        if total_count > 0:
            ...
            fig_risk = create_donut_chart(
                fraud_count,
                legitimate_count
            )

            # ------------------------------------------------
            # FORCE DARK TEXT FOR WHITE PAGE
            # ------------------------------------------------
            ...
            # Make donut labels clearly visible
            if hasattr(fig_risk, "data"):

                for trace in fig_risk.data:
            ...
            st.info(
                "No real-time transactions received yet. "
                "Ensure the Kafka producer is sending data."
            )
    ...
    with c2:
        ...
        st.markdown(
            ...
            <div class="dashboard-panel">
                ...
                <div class="panel-subtitle">
                    Validation metrics from the Random Forest engine
                </div>
        ...
        fig_model = create_model_performance_chart()

        # ----------------------------------------------------
        # FORCE DARK TEXT FOR MODEL CHART
        # ----------------------------------------------------
    ...
    with e1:
        ...
        if view_mode == "Live Real-Time Stream":
            ...
            exp_text = (
                f"The live Kafka stream has evaluated "
                f"<b>{total_count:,}</b> incoming transactions, "
                f"identifying <b>{fraud_count:,}</b> high-risk "
                f"incidents ({fraud_rate:.2f}% stream fraud rate)."
            )

        else:

            exp_text = (
                f"The historical baseline contains "
                f"<b>{HISTORICAL_FRAUD:,}</b> identified fraudulent "
                f"transactions out of "
                f"<b>{HISTORICAL_TRANSACTIONS:,}</b> records "
                f"({HISTORICAL_FRAUD_RATE:.2f}% historical rate)."
            )
        ...
        st.markdown(
            ...
            <div class="insight-card">
                ...
                <div class="insight-text">
                    ...
                    The Random Forest classifier operates at a
                    ROC-AUC of <b>{roc_value:.2f}%</b> and
                    PR-AUC of <b>{pr_value:.2f}%</b>,
                    providing discrimination between fraudulent
    ...
    with e3:
        ...
        drift_note = (
            ...
            if fraud_rate < 5.0
            else "Elevated risk alerts detected in stream"
        )

        precision_value = (
            MODEL_PRECISION * 100
            if MODEL_PRECISION <= 1
            else MODEL_PRECISION
        )
        ...
        st.markdown(
            ...
            <div class="insight-card">
                ...
                <div class="insight-text">
                    ...
                    Operational precision is maintained at
                    <b>{precision_value:.1f}%</b>.
...
# ============================================================
    ...
    st.markdown(
        ...
        <div class="dashboard-header">
            <div>
                <div class="dashboard-title">Real-Time Fraud Detection</div>
                <div class="dashboard-subtitle">
    ...
    if transactions:
        ...
        st.dataframe(pd.DataFrame(table_data), use_container_width=True, height=400, hide_index=True)
    else:
...
# ============================================================
    ...
    st.markdown(
        ...
        <div class="dashboard-header">
            <div>
                ...
                <div class="dashboard-subtitle">
                    Monitor transaction behaviour and fraud patterns across historical data and live streams.
                </div>
            </div>
            <div class="live-badge">
                ● ANALYTICS ACTIVE
    ...
    if amount_chart is not None:
        st.markdown("### Historical Transaction Amount Distribution")
...
# ============================================================
    ...
    st.markdown(
        ...
        <div class="dashboard-header">
            <div>
                <div class="dashboard-title">Adaptive Monitoring</div>
                <div class="dashboard-subtitle">
    ...
    st.metric("Real-Time Transactions Sampled", tx_count)
    st.info("Adaptive monitoring layer actively monitors live data for feature drift, concept drift, and prediction distribution shifts.")
