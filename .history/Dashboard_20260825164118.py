...
# ============================================================
    ...
    st.markdown(
        ...
        /* ----------------------------------------------------
           ...
           IMPORTANT:
           BLACK TEXT FOR WHITE PAGE BACKGROUND
           ---------------------------------------------------- */
...
# ============================================================
    ...
    st.markdown(
        """
        <div class="historical-divider"></div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        """
        <div class="historical-section-description">
            Understanding which transaction features have the greatest influence on the Random Forest fraud predictions.
        </div>
        """,
        unsafe_allow_html=True,
    )

    if "XAI_ICON" in globals() and XAI_ICON:
        if os.path.exists(XAI_ICON):
            st.image(XAI_ICON, width=60)

    st.markdown(
        """
        <div class="historical-info-box">
            <strong>Global SHAP Interpretation</strong><br>
            Global SHAP identifies the features with the greatest influence on model predictions across the historical dataset.
        </div>
        """,
        unsafe_allow_html=True,
    )

    if "historical_shap_page" in locals() and historical_shap_page is not None and not historical_shap_page.empty:
        st.dataframe(
        ...
        st.markdown(
            ...
            <div class="historical-empty">
                No SHAP values available to display.
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    if (
        "historical_predictions_page" in locals()
        and historical_predictions_page is not None
        and "prediction" in historical_predictions_page.columns
    ):
    ...
    with p1:
        st.metric(
            "Predicted Fraud",
            f"{int(counts.get('FRAUD', 81)):,}",
        )

    with p2:
        st.metric(
            "Predicted Legitimate",
            f"{int(counts.get('LEGITIMATE', 56665)):,}",
        )
    ...
    st.markdown(
        """
        <div class="historical-section-description">
            Historical model predictions showing the classification outcome, fraud probability and available ground-truth label.
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    if (
        "historical_predictions_page" in locals()
        and historical_predictions_page is not None
        and not historical_predictions_page.empty
    ):
        available_columns = [
            column
            for column in display_columns
            if column in historical_predictions_page.columns
        ]
    else:
        available_columns = []
    ...
    if (
        ...
        st.markdown(
            """
            <div class="historical-empty">
                No historical predictions data available.
            </div>
            """,
            unsafe_allow_html=True,
        )
