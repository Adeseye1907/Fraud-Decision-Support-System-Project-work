...
<div class="section-header">
    ...
    <p>Fraud Detection & Operational Decision Support System</p>
</div>
""", unsafe_allow_html=True)
...
with col1:
    st.metric(
        label="Transactions",
        value="284,807",
        help="Historical transactions analysed during model development."
    )
...
with col2:
    st.metric(
        label="Fraud Cases",
        value="492",
        help="Confirmed fraudulent transactions in the historical dataset."
    )
...
with col3:
    st.metric(
        ...
        value="0.17%",
        help="Proportion of fraudulent transactions in the historical dataset."
    )
...
with col1:
    st.metric(
        label="ROC-AUC",
        value="96.06%",
        help="Ability of the model to distinguish fraudulent transactions from legitimate transactions."
    )
...
with col2:
    st.metric(
        label="PR-AUC",
        value="81.27%",
        help="Performance of the model on the minority fraud class."
    )
...
st.markdown("### Real-Time Operational Monitoring")

# Get transactions received during the current Streamlit session
live_transactions = st.session_state.get("transactions", [])

transactions_received = len(live_transactions)
...
for transaction in live_transactions:
    ...
    # Support different prediction labels used by the pipeline
    if "FRAUD" in prediction:
        fraud_count += 1

    elif "LEGITIMATE" in prediction or "NORMAL" in prediction:
        legitimate_count += 1


# Calculate live fraud rate
if transactions_received > 0:
    live_fraud_rate = (
        fraud_count / transactions_received
    ) * 100
else:
    live_fraud_rate = 0.0
...
with col3:
    st.metric(
        ...
        value=f"{legitimate_count:,}"
    )

with col4:
    st.metric(
        label="Live Fraud Rate",
        value=f"{live_fraud_rate:.2f}%"
    )


st.caption(
    "Live operational metrics are calculated from transactions received "
    "through the Kafka event stream during the current monitoring session."
)
...
if transactions_received > 0:
    ...
    st.info(
        "Waiting for transactions from the Kafka stream..."
    )
...
st.write(
    "The Random Forest model was evaluated using accuracy, precision, "
    "recall, F1-score, ROC-AUC, and PR-AUC. Because fraudulent transactions "
    "represent a very small proportion of the dataset, precision, recall, "
    "F1-score, and PR-AUC provide more meaningful insights than accuracy alone."
)
...
evaluation_data = {
    ...
    "Interpretation": [
        ...
        "90% of transactions flagged as fraud were actually fraudulent.",
        "77% of fraudulent transactions were detected.",
        "Balance between precision and recall.",
        "Strong ability to distinguish fraud from legitimate transactions.",
        "Strong performance on the minority fraud class."
    ]
}
...
<h4>Evaluation Conclusion</h4>

<p>
The Random Forest model demonstrates strong discriminatory performance,
with a <strong>ROC-AUC of 96.06%</strong> and
<strong>PR-AUC of 81.27%</strong>.
Its <strong>90% precision</strong> indicates relatively reliable
fraud alerts, while the <strong>77% recall</strong> shows that further
improvement may be possible in reducing missed fraudulent transactions.
</p>
...
st.markdown("### Decision Support Pipeline")

pipeline_items = [
    ("Transaction Ingestion", "ACTIVE"),
    ("Kafka Event Streaming", "ACTIVE"),
    ("Fraud Classification", "ACTIVE"),
    ("Explainability Layer", "ACTIVE"),
    ("Adaptive Monitoring", "ACTIVE"),
    ("Decision Support Dashboard", "ACTIVE")
]

for component, status in pipeline_items:

    st.markdown(
        f"""
        <div class="status-row">
            <span>{component}</span>
            <span class="status-good">● {status}</span>
        </div>
        """,
        unsafe_allow_html=True
    )
...
<h4>Fraud Exposure</h4>

<p>
The historical dataset contains <strong>492 fraudulent transactions</strong>
out of <strong>284,807 transactions</strong>, representing a historical
fraud rate of approximately <strong>0.17%</strong>.
</p>

<p>
The low proportion of fraudulent transactions demonstrates the
class-imbalance challenge associated with fraud detection.
</p>
...
<p>
The Random Forest model achieved a <strong>ROC-AUC of 96.06%</strong>
and a <strong>PR-AUC of 81.27%</strong>, indicating strong ability
to distinguish fraudulent transactions from legitimate activity.
</p>

<p>
The PR-AUC is particularly useful in this context because fraudulent
transactions represent a very small proportion of the overall dataset.
</p>
...
<p>
The model currently achieves <strong>77.0% recall</strong> and
<strong>90.0% precision</strong> for the fraud class.
</p>

<p>
This means the system identifies a substantial proportion of fraudulent
activity while maintaining a relatively high level of precision when
generating fraud alerts.
</p>

<p>
The model's predictions should therefore support
<strong>risk-based review and investigation</strong> rather than
automatically replacing human decision-making.
</p>
