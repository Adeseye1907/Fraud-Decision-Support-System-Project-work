...
# ============================================================
    ...
    # Poll live Kafka transaction events
    read_kafka_transactions()
    ...
    st.markdown(
        ...
        <div class="dashboard-header">
            <div>
                <div class="dashboard-title">Adaptive Monitoring & Drift Detection</div>
                <div class="dashboard-subtitle">
                    Monitors incoming transaction behaviour and provides an early indication of whether the operating environment has changed.
                </div>
            </div>
            <div class="live-badge">
    ...
    # --------------------------------------------------------
    # EXTRACT TRANSACTIONS & COMPUTE STATISTICAL DRIFT
    # --------------------------------------------------------
    transactions = list(st.session_state.realtime_transactions)
    transaction_count = len(transactions)

    baseline_fraud_rate = float(HISTORICAL_FRAUD_RATE)  # ~0.17%
    ...
    if transaction_count >= 10:
        ...
        # 1. Evaluate Prediction Class Divergence
        if "prediction" in live_df.columns:
            fraud_count = sum(1 for x in transactions if str(x.get("prediction", "")).upper() == "FRAUD")
            stream_fraud_rate = (fraud_count / transaction_count) * 100

            # Historical baseline is ~0.17%
            if stream_fraud_rate > 20.0:
                drift_score += 0.50
                drift_reasons.append("Severe increase in stream fraud prediction rate (> 20.00%)")
            elif stream_fraud_rate > 10.0:
                drift_score += 0.25
                drift_reasons.append("Moderately elevated fraud prediction rate (> 10.00%)")

        # 2. Evaluate Continuous Risk Probability Divergence
        if "fraud_probability" in live_df.columns:
            probs = pd.to_numeric(live_df["fraud_probability"], errors="coerce").dropna()
            if not probs.empty:
                # Normalize decimals to percentages (e.g., 0.85 -> 85.0%)
                probs = np.where(probs <= 1.0, probs * 100.0, probs)
                mean_probability = float(np.mean(probs))

                if mean_probability > 30.0:
                    drift_score += 0.50
                    drift_reasons.append("Elevated average fraud risk probability (> 30.00%)")
                elif mean_probability > 15.0:
                    drift_score += 0.25
                    drift_reasons.append("Moderately elevated average fraud risk probability (> 15.00%)")

    else:
        drift_reasons.append(
            f"Insufficient real-time transactions ({transaction_count}/10 events sampled). Accumulating stream buffer for reliable monitoring."
        )
    ...
    # Assign operational governance status
    if transaction_count < 10:
    ...
    with k1:
        st.markdown(
            f"""
            <div class="adaptive-kpi-card">
                <div class="adaptive-kpi-label">Sampled Transactions</div>
                <div class="adaptive-kpi-val">{transaction_count:,}</div>
                <div class="adaptive-kpi-sub">Active buffer window</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    with k2:
        st.markdown(
            f"""
            <div class="adaptive-kpi-card">
                <div class="adaptive-kpi-label">Stream Fraud Rate</div>
                <div class="adaptive-kpi-val">{stream_fraud_rate:.2f}%</div>
                <div class="adaptive-kpi-sub">Baseline: {baseline_fraud_rate:.2f}%</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    with k3:
        st.markdown(
            f"""
            <div class="adaptive-kpi-card">
                <div class="adaptive-kpi-label">Mean Stream Risk</div>
                <div class="adaptive-kpi-val">{mean_probability:.2f}%</div>
                <div class="adaptive-kpi-sub">Baseline: {baseline_avg_risk:.2f}%</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    with k4:
        st.markdown(
            f"""
            <div class="adaptive-kpi-card">
                <div class="adaptive-kpi-label">Drift Score & Status</div>
                <div class="adaptive-kpi-val" style="color: {status_color} !important; font-size: 21px;">
                    {drift_score:.2f} ({drift_status})
                </div>
                <div class="adaptive-kpi-sub">Range: 0.00 – 1.00</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    with c1:
        st.markdown(
            """
            <div class="dashboard-panel">
                <div class="panel-title">Fraud Rate Comparison: Baseline vs. Stream</div>
                <div class="panel-subtitle">Compares historical training class prevalence with the live stream.</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    with c2:
        st.markdown(
            """
            <div class="dashboard-panel">
                <div class="panel-title">Stream Risk Probability Concentration</div>
                <div class="panel-subtitle">Distribution of model confidence scores across active transactions.</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        if transaction_count >= 10 and "fraud_probability" in live_df.columns:
            ...
            st.info("Accumulating 10+ transactions to plot the live risk distribution histogram.")
    ...
    if drift_status == "ADAPTATION REQUIRED":
        st.error(
            "🚨 **Adaptation Required**: The monitoring indicators suggest that transaction behaviour has shifted "
            "sufficiently to warrant model review and possible retraining. Proceed to the **Model Adaptation** module."
        )
    elif drift_status == "MONITOR CLOSELY":
        st.warning(
            "⚠️ **Monitor Closely**: Early indications of behavioural change have been detected in the stream. "
            "Continuous surveillance is recommended."
        )
    elif drift_status == "STABLE":
        st.success(
            "✓ **Stable Monitoring State**: Current monitoring indicators do not indicate significant drift. "
            "Model predictions align with historical operating parameters."
        )
    else:
        st.info(
            "ℹ️ **Data Accumulation Phase**: Collect additional streaming events before an adaptation decision can be evaluated."
        )

    if st.button("🔄 Re-evaluate Adaptive Sensors", use_container_width=True):
        st.rerun()
