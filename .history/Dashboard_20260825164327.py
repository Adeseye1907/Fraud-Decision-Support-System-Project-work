...
# ============================================================
    ...
    st.markdown(
        ...
        /* High-visibility headers and subheaders */
        h1, h2, h3, .stSubheader, [data-testid="stSubheader"], [data-testid="stMarkdownContainer"] h3 {
    ...
    # Safe check for XAI_ICON definition and file existence
    ...
    if (
        ...
        fraud_val = f"{int(counts.get('FRAUD', 0)):,}"
        legit_val = f"{int(counts.get('LEGITIMATE', 0)):,}"
    else:
        fraud_val = "81"
        legit_val = "56,665"
    ...
    p1, p2 = st.columns(2)
    with p1:
        st.metric("Predicted Fraud", fraud_val)
    with p2:
        st.metric("Predicted Legitimate", legit_val)
    ...
    if (
        ...
        st.dataframe(
            historical_predictions_page[available_columns] if available_columns else historical_predictions_page,
            use_container_width=True,
            hide_index=True,
        )
    else:
