...
# ============================================================
# 3. HISTORICAL MODEL ANALYSIS (ISOLATED DARK CARDS + CSS)
# ============================================================

elif page == "Historical Model Analysis":

    # Scoped CSS applied directly to fix visibility of headers, metric boxes, and tables
    st.markdown(
        """
        <style>
        /* Scoped dark metric container styling */
        ...
        /* Enforce bright, crisp subheaders on this page */
        ...
        /* Subheader text inside markdown containers */
    ...
    if "MODEL_ICON" in globals() and os.path.exists(MODEL_ICON):
        st.image(MODEL_ICON, width=65)
        
    st.title("Historical Model Analysis")
    st.write("Evaluation and interpretation of the trained Random Forest fraud detection model.")
    st.divider()

    try:
        with open("historical_summary.json", "r") as f:
            historical_summary_page = json.load(f)

        historical_predictions_page = pd.read_csv("historical_predictions.csv")
        historical_shap_page = pd.read_csv("historical_global_shap.csv")
        files_loaded = True

    except Exception as e:
        files_loaded = False
        st.error(f"Historical analysis files could not be loaded: {e}")

    if files_loaded:
        # Load summary metrics
        total_transactions = int(historical_summary_page.get("total_transactions", 0))
        actual_fraud_count = int(historical_summary_page.get("actual_fraud_count", 0))
        fraud_caught = int(historical_summary_page.get("fraud_caught", 0))
        fraud_missed = int(historical_summary_page.get("fraud_missed", 0))
        false_alarms = int(historical_summary_page.get("false_alarms", 0))
        detection_rate = float(historical_summary_page.get("detection_rate", 0))
        roc_auc = float(historical_summary_page.get("roc_auc", 0))
        pr_auc = float(historical_summary_page.get("pr_auc", 0))
        fraud_precision = float(historical_summary_page.get("fraud_precision", 0))
        fraud_recall = float(historical_summary_page.get("fraud_recall", 0))
        fraud_f1 = float(historical_summary_page.get("fraud_f1", 0))
        ...
        with c3:
            st.metric("Fraud Precision", f"{fraud_precision:.2f}")
        with c4:
            st.metric("Fraud Recall", f"{fraud_recall:.2f}")

        st.divider()

        # 2. Historical Test Dataset Section
        st.subheader("Historical Test Dataset")
        d1, d2, d3, d4 = st.columns(4)
        with d1:
            st.metric("Transactions Evaluated", f"{total_transactions:,}")
        with d2:
            st.metric("Actual Fraud", f"{actual_fraud_count:,}")
        with d3:
            st.metric("Fraud Detected", f"{fraud_caught:,}")
        with d4:
            st.metric("Fraud Missed", f"{fraud_missed:,}")

        st.divider()

        # 3. Operational Performance Section
        st.subheader("Operational Fraud Detection Performance")
        ...
        with o3:
            st.metric("Fraud F1-Score", f"{fraud_f1:.3f}")

        st.divider()

        # 4. Global Model Interpretability Section
        st.subheader("Global Model Interpretability")
        if "XAI_ICON" in globals() and os.path.exists(XAI_ICON):
        ...
        if "Feature" in historical_shap_page.columns and "Mean_Abs_SHAP" in historical_shap_page.columns:
            chart_df = (
                historical_shap_page.head(10)
        ...
        # 5. Prediction Outcome Summary Section
        ...
        # 6. Historical Prediction Results Table
        st.subheader("Historical Prediction Results")
        display_columns = ["prediction", "fraud_probability", "actual_label"]
        available_columns = [c for c in display_columns if c in historical_predictions_page.columns]
