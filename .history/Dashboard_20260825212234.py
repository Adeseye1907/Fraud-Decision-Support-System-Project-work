...
# ============================================================
    ...
    st.markdown(
        """
        <style>
        /* Force dark, crisp headers and subheaders */
        h1, h2, h3, h4, .stSubheader, [data-testid="stSubheader"], [data-testid="stMarkdownContainer"] h3 {
            color: #10233f !important;
            font-weight: 750 !important;
            margin-top: 1rem !important;
        ...
        /* Active tab indicator (replaces red with emerald green) */
        div[data-baseweb="tab-highlight"] {
            background-color: #059669 !important;
            height: 3px !important;
        }

        /* Active tab text (replaces red with emerald green) */
    ...
    with tab1:
        ...
        st.markdown(
            '<div class="explorer-caption">Filter and inspect records from the historical fraud dataset by minimum transaction amount.</div>',
            unsafe_allow_html=True,
        )

        if "historical_df" in locals() and historical_df is not None:
            col_filter1, col_filter2 = st.columns([2, 1])
            with col_filter1:
                search_amount = st.number_input(
                    "Minimum Transaction Amount ($)", min_value=0.0, value=0.0, step=10.0
                )
            with col_filter2:
    ...
    with tab2:
        ...
        st.markdown(
            '<div class="explorer-caption">Search for a specific transaction ID processed during the current dashboard session.</div>',
            unsafe_allow_html=True,
        )
        ...
        if query:
            ...
            st.caption(f"Showing all {len(transactions)} active session transactions")
            st.dataframe(pd.DataFrame(transactions), use_container_width=True, hide_index=True)
        else:
            st.info("No live stream transactions recorded in this session yet.")
# ============================================================
# 7. TRENDS & MONITORING
# ============================================================
