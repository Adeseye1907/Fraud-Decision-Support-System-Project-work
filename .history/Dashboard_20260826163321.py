...
# ============================================================
    ...
    # --------------------------------------------------------
    # 1. LIVE SYSTEM COMPONENT STATUS
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # 2. SEQUENTIAL WORKFLOW STAGE CARDS
    # --------------------------------------------------------
    st.markdown("""<div class="section-title">Sequential Decision Architecture</div>""", unsafe_allow_html=True)
    ...
    workflow = [
        ...
        ("02", "Machine Learning", "Random Forest classifier evaluates transaction-level fraud probability and binary risk flags."),
        ("03", "Real-Time Ingestion", "Apache Kafka asynchronously transports transaction payloads and streaming model predictions."),
        ("04", "Explainable AI (XAI)", "SHAP and LIME provide global feature attributions and local case explanations for investigator trust."),
        ("05", "Trends & Monitoring", "Aggregates real-time throughput, transaction volume trends, fraud velocity, and elevated risk activity."),
        ("06", "Adaptive Monitoring", "Continuous Kolmogorov-Smirnov statistical tests identify covariate shifts and concept drift away from baseline."),
        ("07", "Model Adaptation", "Champion-challenger governance workflow evaluates retraining triggers without disrupting active operations."),
        ("08", "Operational Feedback", "Investigators record human adjudication outcomes (Confirmed Fraud, False Positive, Legitimate)."),
        ("09", "Feedback Learning Pipeline", "Reconciles audit feedback and curates high-quality candidate datasets for downstream learning."),
        ("10", "Cost / Value Framing", "Translates model precision and recall into monetary trade-offs, loss prevention, and triage costs."),
        ("11", "Decision Support System", "Central Streamlit portal synthesizes predictions, XAI diagnostics, and governance controls into actionable intelligence.")
    ]
    ...
    for number, title, description in workflow:
        st.markdown(
            ...
            <div class="workflow-step-card">
                <div class="workflow-step-num">{number}</div>
                <div class="workflow-step-content">
                    <div class="workflow-step-header">{title}</div>
